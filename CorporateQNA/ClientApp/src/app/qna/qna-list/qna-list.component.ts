...
export class QnaListComponent implements OnInit, OnDestroy, DoCheck {
  ...
  ngOnInit(): void {
    ...
    this.homeService.RefreshList.subscribe((value) => {
      this.getQuestions();
    });

    this.homeService.QuestionClick.subscribe((response) => {
    ...
    this.homeService.markEvent.subscribe((answer: AnswerViewModel) => {
      for (let i = 0; i < this.questions.length; i++) {
        if (this.questions[i].id == answer.answerOf) {
          if (
            (this.questions[i].askedById ==
              parseInt(sessionStorage.getItem('userId')))
          ) {
            if (answer.markedAsBest == true) this.questions[i].resolved = true;
  ...
  onSubmit() {
    if (this.formGroup.valid && sessionStorage.getItem('userId')) {
      ...
      answer.answeredBy = parseInt(sessionStorage.getItem('userId'));
      answer.answeredOn = new Date();
      answer.description = this.formGroup.value['editorContent']['content'][0][
        'content'
      ][0]['text'];
  ...
  getQuestions() {
    this.apiService
      .getQuestions(parseInt(sessionStorage.getItem('userId')))
      .subscribe((response) => {
        for (let key in response) {
          response[key].askedOn = this.dateTimeService.dateDiff(
            new Date(response[key].askedOn),
            new Date()
          );
        }
        ...
        this.route.paramMap.subscribe((params) => {
          ...
          if(!param)
            return;
          let userId = Number(param.substring(0, param.indexOf('+')));
          if (userId && param.indexOf('+allquestions') > -1)
            this.filterByUser(userId);
          else if (userId && param.indexOf('+answered') > -1) {
            this.apiService.getQuestions(userId).subscribe((response) => {
              for (let key in response) {
                response[key].askedOn = this.dateTimeService.dateDiff(
                  new Date(response[key].askedOn),
                  new Date()
  ...
  getAnswers() {
    this.apiService.getAnswers(this.activeQuestion.id).subscribe((response) => {
      for (let key in response) {
        response[key].answeredOn = this.dateTimeService.dateDiff(
          new Date(response[key].answeredOn),
          new Date()
        );
      }
  ...
  subscribeFilterEvents() {
    this.homeService.FilterEvent.subscribe((filters: questionFilters) => {
      this.questions = this.allQuestions;

      this.questions = this.filterBySearch(filters.search, this.questions);

      if (filters.category != 0) {
        this.questions = this.filterByCategory(
          filters.category,
          this.questions
        );
      }
      ...
      if (filters.show != 'All') {
        if (filters.show == 'My Participations') {
          ...
          this.questions = this.questions.filter((question) => {
            return question.askedById == parseInt(sessionStorage.getItem('userId'));
          });
      ...
      if (filters.sort != 'All') {
        if (filters.sort.toLowerCase() == 'recent') {
          ...
          this.questions = this.questions.filter((ques) => {
            let num = Number(
              ques.askedOn.substring(0, ques.askedOn.indexOf(' '))
            );
            return num <= 10;
          });
